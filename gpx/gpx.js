        ...
        if (deleteWaypoints) {
            ...
            let wpt = og.wpt.filter((point, waypointIndex) => {
                if (waypointIndices === undefined || waypointIndices.includes(waypointIndex)) {
                    let inBounds = point.attributes.lat >= bounds[0].lat &&
...
export class Track extends GPXTreeNode {
    ...
    getStyle() {
        if (this.extensions && this.extensions['gpx_style:line']) {
            if (this.extensions['gpx_style:line']['gpx_style:color']) {
                return Object.assign(Object.assign({}, this.extensions['gpx_style:line']), { ['gpx_style:color']: `#${this.extensions['gpx_style:line']['gpx_style:color']}` });
            }
    ...
    setStyle(style, force = true) {
        ...
        if (style['gpx_style:color'] !== undefined &&
        ...
        if (style['gpx_style:opacity'] !== undefined &&
...
export class TrackSegment extends GPXTreeLeaf {
    ...
    _computeStatistics() {
        ...
        for (let i = 0; i < points.length; i++) {
            ...
            // elevation
            if (i > 0) {
                const ele = statistics.local.elevation.smoothed[i] -
            ...
            if (points[i].extensions) {
                if (points[i].extensions['gpxtpx:TrackPointExtension'] &&
                ...
                if (points[i].extensions['gpxpx:PowerExtension'] &&
        ...
        statistics.global.speed.moving =
            statistics.global.time.moving > 0
                ...
                : 0;
        statistics.local.speed = distanceWindowSmoothingWithDistanceAccumulator(points, 200, (accumulated, start, end) => points[start].time && points[end].time
            ? (3600 * accumulated) /
    ...
    _computeSlope() {
        const points = this.trkpt;
        return distanceWindowSmoothingWithDistanceAccumulator(points, 50, (accumulated, start, end) => {
            var _f, _g;
            return (100 * (((_f = points[end].ele) !== null && _f !== void 0 ? _f : 0) - ((_g = points[start].ele) !== null && _g !== void 0 ? _g : 0))) /
    ...
    // Producers
    replaceTrackPoints(start, end, points, speed, startTime, removeGaps) {
        let og = getOriginal(this); // Read as much as possible from the original object because it is faster
        ...
        if (speed !== undefined || (trkpt.length > 0 && trkpt[0].time !== undefined)) {
            ...
            if (points.length > 0) {
                ...
                if (points[0].time === undefined ||
                    ...
                    if (last.getLatitude() === points[0].getLatitude() &&
...
export class TrackPoint {
    ...
    toTrackPointType(exclude = []) {
        ...
        if (this.extensions) {
            ...
            if (this.extensions['gpxpx:PowerExtension'] &&
                this.extensions['gpxpx:PowerExtension']['gpxpx:PowerInWatts'] &&
...
export class Waypoint {
    ...
    equals(other) {
        if (this.attributes.lat !== other.attributes.lat ||
        ...
        if ((this.time === undefined && other.time !== undefined) ||
    ...
    mergeWith(other) {
        ...
        this.global.time.start =
            this.global.time.start !== undefined && other.global.time.start !== undefined
                ? new Date(Math.min(this.global.time.start.getTime(), other.global.time.start.getTime()))
                : ((_f = this.global.time.start) !== null && _f !== void 0 ? _f : other.global.time.start);
        this.global.time.end =
            this.global.time.end !== undefined && other.global.time.end !== undefined
                ? new Date(Math.max(this.global.time.end.getTime(), other.global.time.end.getTime()))
                : ((_g = this.global.time.end) !== null && _g !== void 0 ? _g : other.global.time.end);
...
export function getElevationDistanceFunction(statistics) {
...
function distanceWindowSmoothingWithDistanceAccumulator(points, distanceWindow, compute) {
    return distanceWindowSmoothing(points, distanceWindow, (index) => index > 0
        ? distance(points[index - 1].getCoordinates(), points[index].getCoordinates())
        : 0, compute, (index) => distance(points[index].getCoordinates(), points[index + 1].getCoordinates()));
}
